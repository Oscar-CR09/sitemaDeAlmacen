...
public class PersonaDAO implements CrudPaginadoInterface<Persona>{
    ...
    public List<Persona> listar(String texto, int totalPorPagina, int numPagina) {
        ...
        List<Persona> registros=new ArrayList();
        try {
            ps=CON.conectar().prepareStatement("Select p.id, p.tipo_persona, p.nombre, p.tipo_documento, p.num_documento, p.direccion, p.telefono, p.email, p.activo FROM persona p WHERE p.nombre LIKE ?  AND tipoPersona=? ORDER BY p.id ASC LIMIT ?, ?");
            ps.setString(1, "%"+texto+"%");
    ...
    public int total() {
        int totalRegistro=0;
        try {
            ps=CON.conectar().prepareStatement("SELECT COUNT(id) AS total_registros FROM persona"); 
            rs=ps.executeQuery();
            while (rs.next()) {                
