...
public class UsuarioDAO implements CrudPaginadoInterface<Usuario>{
    ...
    public List<Usuario> listar(String texto, int totalPorPagina, int numPagina) {
     ...
     public Usuario login(String email, String clave){
         Usuario usu =null;
         try {
             ps=CON.conectar().prepareStatement("Select u.id, u.rol_id, r.nombre as rol_nombre, u.nombre, u.tipo_documento, u.num_documento, u.direccion, u.telefono, u.email, u.activo FROM usuario u inner join rol r ON u.rol_id=r.id  WHERE u.email=? clave=?");
    ...
    public int total() {
        int totalRegistro=0;
        try {
            ps=CON.conectar().prepareStatement("SELECT COUNT(id) AS total_registros FROM usuario"); //"SELECT  COUNT (id) FROM categoria"
            rs=ps.executeQuery();
            while (rs.next()) {                
